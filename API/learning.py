...
r = requests.get("https://groups.google.com/a/ksvs.kh.edu.tw/forum/feed/learning/msgs/rss.xml?num="+str(items_number),headers=headers)
...
def getitems():
    ...
    for i in range(0, (int)(len(urls))):
        if True:
            print('正在處理..'+str(urls[i].getText()).replace('/d/topic/learning/','/g/learning/c/'))
            print("counter: " + (str(counter)))
            resoult[counter][5] = (str(urls[i].getText()).replace('/d/topic/learning/','/g/learning/c/'))
...
def updateNews():
    ...
    with io.open('json/learning.json', mode='w', encoding='utf8') as json_file:
        json.dump(resoult, json_file, ensure_ascii=False)

def getJson():
    with io.open('json/learning.json', mode='r', encoding='utf8') as json_file:
