...
r = requests.get("https://groups.google.com/a/ksvs.kh.edu.tw/forum/feed/parent/msgs/rss.xml?num="+str(items_number),headers=headers)
...
def getitems():
    ...
    for i in range(0, (int)(len(urls))):
        if True:
            print('正在處理..'+str(urls[i].getText()).replace('/d/topic/parent/','/g/parent/c/'))
            print("counter: " + (str(counter)))
            resoult[counter][5] = (str(urls[i].getText()).replace('/d/topic/parent/','/g/parent/c/'))
...
def updateNews():
    ...
    with io.open('json/parent.json', mode='w', encoding='utf8') as json_file:
        json.dump(resoult, json_file, ensure_ascii=False)
def getJson():
    with io.open('json/parent.json', mode='r', encoding='utf8') as json_file:
